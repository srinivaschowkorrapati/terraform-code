...
locals {
  config_name_3 = "app-intg-platform-${var.env}-retail-corp-broker-3"
}

# Configuration file Broker  3
resource "aws_mq_configuration" "retail_corp_mq_configuration_3" {
  description    = "Configuration for retail corp MQ : AMQBroker 3"
  name           = local.config_name_3
...
<broker schedulePeriodForDestinationPurge="10000" xmlns="http://activemq.apache.org/schema/core">
  ...
  <networkConnectors>
    ...
    <networkConnector conduitSubscriptions="false" consumerTTL="1" messageTTL="-1" name="QueueConnectorConnectingToBroker2"  uri="static:(${var.broker_2_uri})" userName="Administrator">
      <excludedDestinations>
        <topic physicalName="&gt;"/>
      </excludedDestinations>
    </networkConnector>
    <networkConnector conduitSubscriptions="true" consumerTTL="1" messageTTL="-1" name="TopicConnectorConnectingToBroker2"  uri="static:(${var.broker_2_uri})" userName="Administrator">
      <excludedDestinations>
        <queue physicalName="&gt;"/>
      </excludedDestinations>
    </networkConnector>
